...
for i in positive_set:
    ...
    tmp=main_final.analysis_lsde("../structures/criminality/"+i,name_address_dic[i],False)
    tmp['tag']=1
    print(tmp)
    output_list.append(tmp)
with open("result_positive.txt","a+") as f:
    f.write(str(output_list))
output_list=[]
for j in negative_set:
    address=j.split('_')[1][:-8]
    print(negative_map[address])
    tmp=main_final.analysis_lsde("../structures/donor/"+j,negative_map[address],False)
    tmp['tag']=0
    output_list.append(tmp)
with open("result_negative.txt","a+") as f:
    f.write(str(output_list))